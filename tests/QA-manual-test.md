...
**REQUIRED: Node.js (latest versions)** \
**Contact the Developers for Firebase Access**
...
Verify new patient accounts are created with unverified status and verification email sent.
* **Steps:**
...
* **Expected Result:**
  ...
  **the verification may be found on the SPAM category folder**

### Test Case 1.2: Empathetic Login Rate-Limiter (Brute-Force Protection)
Verify that 3 consecutive failed login attempts trigger an empathetic 30-second security timeout.
* **Steps:**
  1. Navigate to `/src/pages/auth/login.html`.
...
Verify password reset link requests are throttled with a 60-second cooldown.
* **Steps:**
...
Verify that account deletion requires password verification and completely purges all Firestore records.
* **Steps:**
...
## Interactive Calendar & Appointment Matrix

### Test Case 2.1: Uniform 140px Calendar Grid & Visual Balance
Verify all calendar grid cells maintain strict mathematical height uniformity.
* **Steps:**
...
Verify clicking a date cell or `+X more...` button opens the unified modal with full CRUD capabilities.
* **Steps:**
...
Verify clicking an individual appointment pill immediately opens the Edit modal.
* **Steps:**
...
Verify dragging an unverified/verified client card onto a date cell schedules an appointment interactively.
* **Steps:**
...
Verify that dragging an existing appointment pill to the bottom trash zone prompts a deletion sequence.
* **Steps:**
...
Verify the system prevents accidental deletion of the last remaining administrator.
* **Steps:**
...
Verify bulk user deletion executes in parallel without UI lag.
* **Steps:**
...
## Patient Portal & Dynamic Announcements

### Test Case 4.1: Live Announcements Broadcast
Verify admin announcements appear in real-time on patient dashboards.
* **Steps:**
...
Verify patient calendar displays appointments with privacy preservation.
* **Steps:**
...
## Telemetry & Developer Analytics

### Test Case 5.1: Developer Analytics Toggle
Verify the Developer Analytics button is hidden by default and can be toggled on per-admin.
* **Steps:**
...
## Speed, Optimization & Tactile User Experience

### Test Case 6.1: Zero-Latency Payload Rejection
Verify that oversized payloads are instantly rejected by the backend without crashing the client browser.
* **Steps:**
  ...
  3. Attempt to save the appointment by clicking the **Save Appointment** button on the UI.
* **Expected Result:**
...
Verify that interactive elements like Drag-to-Delete maintain a 60fps frame rate without layout thrashing.
* **Steps:**
...
Verify the Vanilla JS architecture achieves near-perfect performance scores.
* **Steps:**
